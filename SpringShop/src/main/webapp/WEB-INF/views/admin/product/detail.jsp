									...
									<div class="card-body">
										<div class="form-group">
											<label for="category">상위 카테고리 분류</label> <select
												...
												%>
												<option value="<%=category.getCategory_idx()%>" <% if(product.getCategory().getCategory_idx()==category.getCategory_idx()) out.print("selected"); %>><%=category.getCategory_name()%></option>
												<%
													}
												%>
											</select>
										...
										<div class="form-group">
											<label for="product_name">상품명</label> <input type="text"
												...
												placeholder="상품명을 입력하세요.." value="<%= product.getProduct_name() %>">
										</div>
										<div class="form-group">
											<label for="brand">브랜드명</label> <input type="text"
												class="form-control" id="brand" placeholder="브랜드명을 입력하세요.." value="<%= product.getBrand() %>">
										</div>
										<div class="form-group">
											<label for="price">가격</label> <input type="number"
												class="form-control" id="price" placeholder="가격을 입력하세요.." value="<%= product.getPrice() %>">
										</div>
										<div class="form-group">
											<label for="discount">할인된 가격</label> <input type="number"
												class="form-control" id="discount"
												placeholder="할인된 가격을 입력하세요.." value="<%= product.getDiscount() %>">
										</div>
										<div class="form-group">
											<label for="detail">세부 사항</label>
											<textarea id="detail" name="editordata"><%= product.getDetail() %></textarea>
										</div>
										<div class="form-group">
