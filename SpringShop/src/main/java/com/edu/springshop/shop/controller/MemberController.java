...
public class MemberController {
	...
	public ModelAndView callback(HttpServletRequest request, HttpSession session) {
		...
		//code + clientID + clientSecret = 토큰
		OAuth2Operations operations = googleLogin.getGoogleConnectionFactory().getOAuthOperations();
		
		//넘겨받은 코드를 이용하여 권한객체를 반환받는다
		AccessGrant grant = operations.exchangeForAccess(code, googleLogin.getOAuth2Parameters().getRedirectUri(), null);
		
		String accessToken = grant.getAccessToken();
		logger.info("accessToken : " + accessToken);
		
		Connection<Google> con = googleLogin.getGoogleConnectionFactory().createConnection(accessGrant);
		Google google = con.getApi();
		/*
		 	1) 토큰을 이용하여 사용자의 정보를 접근한다
		 		Google 객체를 얻으면 됨.. (Connection 객체로 얻어온다)
		 	2) 토큰을 통해 얻은 사용자의 정보를 MemberDTO에 담는다
		 	3) session에 DTO를 담아서 로그인 처리를 해준다
		 	4) 로그인 후 보게 될 페이지를 보여준다
		 */
